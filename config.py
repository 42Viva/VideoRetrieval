...
ASSETS_PATH = (
    r"E:\Projects\MaterialSearch\data",
    # r"D:/照片",  # Windows系统用这种写法
)  # 素材所在的目录，绝对路径
SKIP_PATH = (
    r'E:\Downloads\iCloud 照片',
)  # 跳过扫描的目录，绝对路径
...
MODEL_NAME = "openai/clip-vit-large-patch14"  # CLIP模型
TEXT_MODEL_NAME = "IDEA-CCNL/Taiyi-CLIP-Roberta-large-326M-Chinese"  # 中文模型，需要和CLIP模型配套使用，如果LANGUAGE为English则忽略此项
DEVICE = "cuda"  # 推理设备，cpu/cuda/mps，建议先跑benchmark.py看看cpu还是显卡速度更快。因为数据搬运也需要时间，所以不一定是GPU更快。
DEVICE_TEXT = "cuda"  # text_encoder使用的设备，如果LANGUAGE为English则忽略此项。
